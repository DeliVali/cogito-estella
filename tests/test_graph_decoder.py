...
def test_non_autoregressive_single_forward():
    # a single pass yields the whole graph (no time/step dimension)
    cfg = GraphDecoderConfig()
    dec = GraphDecoder(cfg)
    concept = torch.randn(2, cfg.concept_dim)
    out = dec(concept)
    # the number of output "positions" is fixed (K nodes), independent of content
...
def test_param_count_is_light():
    ...
    assert n < 5e6  # << the SONAR decoder's 605M
...
def test_decode_triples_recovers_planted_graph():
    # build logits encoding one clear triple: node0(label 3) --rel 1--> nodo1(label 7)
    B, K, V, R = 1, 4, 16, 4
