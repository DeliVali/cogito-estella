...
def test_ce_discriminates_true_vs_random():
    """A sentence's true embedding must yield much lower CE than ano aleatorio."""
...
def test_loss_sum_matches_mean():
    """loss_sum / n_tokens must match loss() (per-token mean) on un solo batch."""
    from cogito_estella.model.sonar_loss import SonarCELoss
...
def test_gradient_flows_to_embedding_but_not_decoder():
    ...
    # the decoder stays frozen: no parameter accumulates gradient
