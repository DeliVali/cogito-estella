...
def test_encoder_amortized_paid_once():
    # the encode is paid once (independent of n_epochs in the returned term)
...
def test_concept_inference_dominated_by_decoder():
    # with large N_dec, per-token inference cost is dominated by the decoder
    ...
    assert per_tok < 2 * 600_000_000 + 2 * 1_000_000  # + small CT term


def test_graph_decoder_has_no_length_factor():
    # the graph decoder is O(1): its cost does NOT scale with tokens_per_concept
    ...
    assert bd2.sonar_decoder == bd.sonar_decoder  # invariant to text length
...
def test_graph_decoder_much_cheaper_than_sonar_text():
    ...
    assert gr < txt / 50   # at least 50x cheaper per concept


def test_context_processing_favors_compression():
    # compressing the context ~L_tok x cuts prefill cost (linear and quadratic)
